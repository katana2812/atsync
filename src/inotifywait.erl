...
event_manager_loop(Wrapper, {_Node, GenEventServer}=Manager) ->
    receive
        { Wrapper, Data } ->
            gen_event:notify(GenEventServer, Data)
    end,
...
parse_options_events(Events) ->
    ...
    ApplyEvents = lists:foldl(
        fun(E, Acc) ->
            case lists:member(E, Allowed) of
                true -> [E|Acc];
                false -> Acc
            end
        end,
    ...
    ApplyEvents1 = case ApplyEvents of
        []     -> Allowed;
        _Other -> ApplyEvents
    end,
    string:join([ atom_to_list(E) || E <- ApplyEvents1 ], ",").
...
inotify_port(Monitored, Options) ->
    ...
    Args = parse_options(Options) ++ " --format \"%w|%e|%f\" " ++ string:join(Monitored, " "),
